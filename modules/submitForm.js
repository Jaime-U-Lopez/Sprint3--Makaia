...
import postDataFetch from "../fetch/postData.js";
import putDataFetch from "../fetch/putData.js";
...
export const submitForm = async (form, url, editForm) => {
  form.addEventListener("submit", async (e) => {
    ...
    valuesForm.forEach((valueInput) => {
      if (valueInput.id) {
        infoHouse[valueInput.id] = valueInput.value;
      }
    });
    ...
    const noHayCamposVacios = validacionCamposForm(arrayLabels, infoHouse);
    //Acción para editar
    if (noHayCamposVacios && editForm) {
      await putDataFetch(url, infoHouse);
      Swal.fire(
        "¡Excelente!",
    ...
    if (noHayCamposVacios && !editForm) {
      await postDataFetch(url, infoHouse);
      Swal.fire(
        "¡Excelente!",
...
const validacionCamposForm = (arrayLabels, infoHouse) => {
  ...
  let keyStr = "";
  for (const key in infoHouse) {
    const propertyHouse = infoHouse[key];
    console.log(propertyHouse)
    console.log(infoHouse)
